...
from rio.utilities import pad


def test_without_icy_repeats():
...
def test_validate_icy_stream_good():
    ...
    assert icy_data_stream == validated


def test_validate_icy_stream_bad():
    ...
    validated = list(validate_icy_stream(icy_data_stream))
    assert icy_data_stream[:3] == validated


def test_takewhile_tags():
    icy_data_stream = [
        IcyData('a', '0'),
        IcyData('b', '1'),
        IcyData('c', '2', set(['AD'])),
    ...
    assert icy_data_stream[:2] == list(result)


def test_empty_parse_icy():
...
def test_larger_rebuffer_icy():
    """ Rebuffer to a larger ICY_METAINT. """
    yield _rebuffer_tester, 8, 12


def test_smaller_rebuffer_icy():
    """ Rebuffer to a smaller ICY_METAINT. """
    yield _rebuffer_tester, 12, 8


def test_reconstruct_icy():
...
def _rebuffer_tester(old_metaint, new_metaint):
    chunks = 8
